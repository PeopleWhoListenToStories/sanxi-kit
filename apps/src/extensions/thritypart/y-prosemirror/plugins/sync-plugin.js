...
const createTypeFromTextOrElementNode = (node, mapping) =>
  Array.isArray(node) ? createTypeFromTextNodes(node, mapping) : createTypeFromElementNode(node, mapping);
...
const equalYTypePNode = (ytype, pnode) => {
  if (ytype instanceof Y.XmlElement && !(Array.isArray(pnode)) && matchNodeName(ytype, pnode)) {
  ...
  return ytype instanceof Y.XmlText && Array.isArray(pnode) && equalYTextPText(ytype, pnode);
};
 ...
 */
const matchNodeName = (yElement, pNode) => !(Array.isArray(pNode)) && yElement.nodeName === pNode.type.name;
